...
export default function ResponsiveDashboard() {
  ...
  const regularCards = cards.filter(card => 
    !externalLinkKeys.some(key => 
      sidebarGroups.find(group => group.label === "روابط خارجية")?.items.find(item => item.key === key)?.title === card.title
    )
  );
    ...
    <div dir="rtl" className={rootClass} style={backgroundStyle}>
          ...
          <div className="h-16 flex items-center gap-3">
            ...
            <div className="ms-1 flex items-center gap-3">
              ...
              <span 
                ...
                style={{ color: '#D11C69', display: 'none' }}
              >
                درعة
              </span>
            </div>
      ...
      <div className="py-6 flex gap-6">
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 max-w-7xl">
          {renderContent()}
          <footer className="mt-10 text-center text-xs text-neutral-500 dark:text-neutral-400">النسخة الاحتياطية v3 - إزالة الروابط الخارجية المكررة فعلياً</footer>
        </main>
      </div>
